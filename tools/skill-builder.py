...
Usage:
  python skill-builder.py [--source h1|github|all] [--program HANDLE]
                          [--vuln-type TYPE] [--limit N] [--out DIR]

No ANTHROPIC_API_KEY needed — uses `claude -p` CLI (OAuth).
"""
...
def generate_skill(vuln_class: str, reports: list[dict]) -> str:
    """Send grouped reports to Claude via `claude -p` CLI and get a skill file back."""
    ...
    for attempt in range(3):
        try:
            ...
            print(f"[!] claude -p returned code {result.returncode}: {result.stderr[:200]}")
...
def main():
    ...
    # Verify claude CLI is available (OAuth-based, no API key needed)
    try:
        subprocess.run(["claude", "--version"], capture_output=True, check=True, timeout=5)
    except (FileNotFoundError, subprocess.CalledProcessError):
        print("[!] `claude` CLI not found. Install Claude Code: npm install -g @anthropic-ai/claude-code")
        sys.exit(1)
